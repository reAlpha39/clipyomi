...
- **~~`scrollWidth` rounds against the popup's own layout.~~ Under-diagnosed, and found for real in the whole-branch review (C3) — now fixed.** The error is not rounding. `#tooltip` is `height: 100%` of the window, and per CSSOM-View `scrollHeight`/`scrollWidth` return the larger of the padding box and the content extent — so a measurement taken off that element is **floored at the window's own inner size** and can never report anything smaller. The height therefore latches at the tallest entry ever hovered (a one-sense word after a twenty-sense one is a near-full-screen empty box, recovering at 2px per hover), and the width shrinks by the border width on every round trip for the life of the process, since `.tt-line`'s `overflow-wrap: anywhere` makes `scrollWidth` always exactly `clientWidth`. The fix measures the `div.tt` content child, which is `height: auto`, and adds back the chrome the client box excludes; `e2e/popover.spec.ts` now reads the emitted payload, which nothing in either suite did before.
